...
export default function AuthPage() {
  ...
  useEffect(() => {
    ...
    // Auto-detect role switching parameters (?role=listener or ?role=broadcaster)
    const targetRole = searchParams.get('role');
    if (targetRole === 'listener' || targetRole === 'broadcaster') {
      setRole(targetRole);
    }

    // Auto-open Sign Up view if a user arrives from pricing plans or call-to-actions
    const triggerSignup = searchParams.get('signup') === 'true' || searchParams.has('plan') || searchParams.has('trial');
    if (triggerSignup) {
      setMode('signup');
      // If they arrived with pricing metadata, they are definitely setting up a broadcaster profile
  ...
  const handleAuthSubmit = async (e: FormEvent) => {
    ...
    try {
      if (mode === 'signup') {
        // 2. Fire registration request to Supabase Auth
        ...
        if (authData?.user) {
          // ================= 2. SUBSCRIPTION METADATA INTERACTION =================
          const searchParams = new URLSearchParams(window.location.search);
          const planSelected = searchParams.get('plan') || 'none';
          const isTrialRequested = searchParams.get('trial') === 'true';

          // Inject profile parameters directly including custom metadata role & plan metrics
          const { error: profileError } = await supabase
            ...
            .insert({
              ...
              // Provision plans if account type is set to broadcaster
              ...
              trial_ends_at: role === 'broadcaster' && isTrialRequested 
                ? new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString() // 7-Days window duration
                : null
            });
       ...
       const { data: logInData, error: loginError } = await supabase.auth.signInWithPassword({
        ...
        if (logInData?.user) {
          // Fetch user profile safely using .maybeSingle() to prevent multi-row coercion errors
          const { data: profile, error: profileFetchError } = await supabase
            .from('profiles')
            .select('role')
            .eq('id', logInData.user.id)
            .maybeSingle(); // <--- CHANGE .single() TO .maybeSingle() HERE

          if (profileFetchError) throw profileFetchError;

          // Route to matching workflow instance or fallback safely
          if (profile?.role === 'broadcaster') {
            router.push('/broadcaster');
          } else if (profile?.role === 'listener') {
            router.push('/listener');
          } else {
            // Fallback default routing if the profile row was deleted or lacks an explicitly assigned role
            router.push('/broadcaster');
          }
        }
      }
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4 font-sans text-slate-100">
      <div className="w-full max-w-md bg-slate-950 border border-slate-800 rounded-2xl p-8 shadow-2xl space-y-6">
        ...
        {/* Top Typography Header branding */}
        ...
        {/* Error message logging notification block */}
        {errorMessage && (
        ...
        <form onSubmit={handleAuthSubmit} className="space-y-4">
          ...
          {mode === 'signup' && (
            <>
              {/* Sign Up Exclusive Input Parameter Modules */}
          ...
          {/* Primary Authentication Fire Button */}
          <button
            type="submit"
            disabled={loading}
